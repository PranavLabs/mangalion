...
export default function Home() {
  ...
  const [searching, setSearching] = useState(false); // Search specific loader
  
  // Provider State
  const [provider, setProvider] = useState('mangapill');

  // --- 1. FETCH HOME DATA (Popular & Latest) ---
  ...
  // --- 2. EXECUTE SEARCH (The actual API call) ---
  ...
  useEffect(() => { 
      // Only fetch home data if not searching
      if (search === '') {
          fetchHomeData(); 
      }
  }, [fetchHomeData, search]); 

  // --- 4. EFFECT: Debounced Live Search ---
  useEffect(() => {
  ...
  const MangaCard = ({ m }: { m: any }) => (
    <Link href={`/manga/${m.id}?provider=${provider}`} className="group relative">
        <div className="aspect-[2/3] rounded-3xl overflow-hidden bg-white/5 border border-white/10 shadow-2xl relative transition-all duration-500 group-hover:-translate-y-2 group-hover:shadow-pink-500/20 group-hover:border-pink-500/30">
            ...
            {m.image ? (
                <img 
                    src={`/api/proxy?url=${encodeURIComponent(m.image)}&source=${provider}`} 
                    className="object-cover w-full h-full opacity-80 group-hover:opacity-100 transition-opacity duration-500"
                    alt={m.title}
                    loading="lazy"
                />
            ) : (
                <div className="w-full h-full bg-gradient-to-br from-purple-900/40 to-blue-900/40 flex items-center justify-center p-4">
                    <span className="text-white/20 text-xs font-bold uppercase tracking-widest text-center">No Cover</span>
                </div>
            )}
  ...
  return (
    <div className="min-h-screen bg-[#0f0f11] text-white selection:bg-pink-500 selection:text-white overflow-x-hidden relative bg-[url('/noise.png')]">
      ...
      <header className="sticky top-0 z-50 w-full px-4 md:px-8 py-4 bg-[#0f0f11]/70 backdrop-blur-xl border-b border-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.1)]">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
              ...
              <div className="flex items-center">
                  ...
                  >
                     <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037 13.48 13.48 0 0 0-1.291 2.665 18.354 18.354 0 0 0-4.135 0 13.46 13.46 0 0 0-1.29-2.665.074.074 0 0 0-.079-.037 19.736 19.736 0 0 0-4.885 1.515.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.076.076 0 0 0-.04.106c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.086 2.157 2.419 0 1.334-.956 2.42-2.157 2.42zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.086 2.157 2.419 0 1.334-.946 2.42-2.157 2.42z"/></svg>
                  </a>
              </div>
          </div>
      </header>
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-8 relative z-10 mt-4">
            ...
            <div className="flex flex-col md:flex-row gap-4 p-3 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2rem] shadow-2xl ring-1 ring-black/5">
                <div className="relative group w-full md:w-auto">
                    ...
                    onChange={(e) => {
                         ...
                         if(search) executeSearch(search); // Re-search if provider changes
                    }}
        ...
        {!loading && (
            <>
                {/* 1. SEARCH RESULTS (Only visible if searching) */}
                {search.trim() !== '' && (
                    <div className="mb-12">
                        <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
                             <span className="text-pink-500">🔎</span> Search Results
                             {searching && <span className="text-sm text-white/40 font-normal animate-pulse">(Searching...)</span>}
                        </h2>
                        ...
                        {!searching && searchResults.length === 0 && (
                            <div className="text-center py-20 text-white/30 border border-white/5 rounded-3xl bg-white/5">
                ...
                {/* 2. HOME FEED (Hidden if searching) */}
                {search.trim() === '' && (
