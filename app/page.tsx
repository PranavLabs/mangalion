...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#0f0f11] text-white selection:bg-pink-500 selection:text-white overflow-x-hidden relative bg-[url('/noise.png')]">
                  ...
                  <div className="relative h-10 w-10 rounded-xl overflow-hidden shadow-[0_0_20px_rgba(236,72,153,0.5)] border border-white/10 group-hover:scale-105 transition-transform">
                      ...
                      <Image 
                        ...
                        onError={(e) => {
                          ...
                          e.currentTarget.parentElement?.classList.add('bg-gradient-to-br', 'from-pink-500', 'to-blue-500');
                        }}
                      />
                      {/* Fallback background if no logo exists yet */}
      ...
      {/* MAIN CONTENT START - Added margin top */}
      <div className="max-w-7xl mx-auto p-4 md:p-8 relative z-10 mt-4">
        
        {/* SEARCH BAR - Adjusted top position so it sits below header */}
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 space-y-4">
             ...
             <div className="text-white/50 font-light tracking-widest animate-pulse">CONNECTING TO KOMIK...</div>
          </div>
        ) : (
          <>
            {mangaList.length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    {mangaList.map((m) => (
                    <Link href={`/manga/${m.id}?provider=${provider}`} key={m.id} className="group relative">
